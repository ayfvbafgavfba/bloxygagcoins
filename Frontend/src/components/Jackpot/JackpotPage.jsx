...
export default function JackpotPage() {
  ...
  const fetchJackpotData = useCallback(async () => {
    const res = await fetch(`${config.api}/jackpot`, {
      mode: "cors",
      method: "GET",
    });
  ...
  const handleModalOpen = useCallback(() => {
    if (userData != null && isUserBanned(userData.username)) {
      return toast.error("You are banned from joining Jackpot games.");
    }
    if (jackpotData.result == null) {
      setModalState(
        <JackpotJoining
          ...
          onJoinSuccess={fetchJackpotData}
        />
      );
    }
  }, [fetchJackpotData, jackpotData, userData]);

  const handleJackpotUpdate = useCallback(
    async (data) => {
      console.log("[client socket] JACKPOT_UPDATE received", data);
      if (data?.gameData && Array.isArray(data?.entries)) {
        setJackpotData(data.gameData);
        setJackpotEntries(data.entries);
      } else {
  ...
  const rotationPercentCalc =
    jackpotData.value > 0 && jackpotData.result != null
      ? jackpotData.result / jackpotData.value
      : 0;
      ...
      <div className="JackpotPage">
        <div className="Container">
          <div className="WheelBG">
            ...
            <div
              className={`Wheel ${jackpotData.result != null ? "Spin" : ""}`}
              style={
            ...
            <div
              className={`WheelMask ${jackpotData.result != null ? "Spin" : ""}`}
              style={
                jackpotData.result != null
                  ? {
                      transform: `rotate(${rotationDEGCalc}deg)`,

  ...
  useEffect(() => {
    fetch(`${config.api}/marketplace/listings`, {
      ...
      if (Array.isArray(JSON.parse(localStorage.getItem("CartItems")))) {
        ...
        // render initial batch and set observation index so observer can paginate
        const initial = Math.min(20, sortedReturnOther.length);
        setObservationIndex(initial);
        setRenderOtherListings(sortedReturnOther.slice(0, initial));
        ...
        const initial2 = Math.min(20, sortedOther.length);
        setObservationIndex(initial2);
        setRenderOtherListings(sortedOther.slice(0, initial2));
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting) {
          ...
          let newIndex = Math.min(observationIndexDebounced + 20, otherListings.length);
          setObservationIndex(newIndex);
          setRenderOtherListings(otherListings.slice(0, newIndex));
          if (newIndex >= otherListings.length) {
            setLoadingState(false);
          }
        }
      },
        ...
        <div className="Filter">
          ...
          <div className="Info">
            <div className="Copyright">
              <h3>
                � {new Date().getFullYear()} BloxyGAG.com All Rights Reserved
              </h3>
              <p className="SubText">
                BloxyGAG.com is not affiliated, associated, or partnered with

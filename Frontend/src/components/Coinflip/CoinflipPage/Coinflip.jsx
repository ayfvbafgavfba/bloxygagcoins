...
export default function Coinflip() {
  ...
  const fetchCoinflipData = useCallback(async () => {
    const res = await fetch(`${config.api}/coinflips`, {
      mode: "cors",
      method: "GET",
    });
  ...
  const handleCoinflipsUpdate = useCallback(
    (data) => {
      console.log("[client socket] COINFLIP_UPDATE received", data);
      setActiveFlips(data.activeFlips);
      if (filteredGame == "All") {
        setFilteredGames(data.activeFlips);
      } else {
  ...
  const handleCreationModal = useCallback(() => {
    if (userData != null) {
      if (isUserBanned(userData.username)) {
        toast.error("You are banned from creating Coinflip games.");
        return;
      }
      setModalState(
        <CoinflipCreation
          ...
          onCreateSuccess={fetchCoinflipData}
        />
      );
    }
  }, [userData]);

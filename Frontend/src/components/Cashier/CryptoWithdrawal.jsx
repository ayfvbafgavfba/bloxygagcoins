...
export default function CryptoWithdrawal({ closeModal, changeModal }) {
  ...
  const [currency, setCurrency] = useState("LTC"); // Default to Litecoin
  const [currencyLong, setCurrencyLong] = useState("LITECOIN (LTC)");
  const [network, setNetwork] = useState("litecoin"); // Default to Litecoin network
  ...
  const handleWithdrawCrypto = useCallback(async () => {
    if (address == "") {
      ...
      toast.error("Your withdrawal amount must be over $0.01");
    } else {
      setIsLoading(true);

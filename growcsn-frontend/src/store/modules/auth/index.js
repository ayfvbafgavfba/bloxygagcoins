...
const actions = {
    ...
    async authSendRobloxVerifyStart({ getters, commit, dispatch }, data) {
        if(getters.authSendLoginLoading === true) { return; }
        commit('auth_set_send_login_loading', true);

        try {
            const res = await axios.post('/auth/roblox/verify-start', data);
            return res.data;
        } catch(err) {
            if(err.response !== undefined && err.response !== null) {
                dispatch('notificationShow', err.response.data.error);
    ...
    async authSendRobloxVerifyBio({ getters, commit, dispatch }, data) {
        if(getters.authSendLoginLoading === true) { return; }
        commit('auth_set_send_login_loading', true);

        try {
            const res = await axios.post('/auth/roblox/verify-bio', data);
            if(res.data.success) {
                commit('auth_set_user', res.data.user);

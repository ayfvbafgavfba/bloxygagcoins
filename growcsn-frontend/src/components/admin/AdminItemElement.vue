<template>
    <div class="admin-item-element">
        <div class="element-info">
            <img v-bind:src="localImage(imageSrc)" @error="onImgError($event)" />
            <div class="info-text">
                {{ item.item.name }}
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            localImage(src) {
                if(!src) return require('@/assets/img/cashier/placeholder.png');
                try {
            ...
            onImgError(event) {
                try {
                    event.target.src = require('@/assets/img/cashier/placeholder.png');
                } catch (e) {}
            }
        },
        computed: {
            imageSrc() {
                try {
                    return this.item && this.item.item && this.item.item.image
                        ? this.item.item.image
                        : require('@/assets/img/cashier/placeholder.png');
                } catch (e) {
                    return require('@/assets/img/cashier/placeholder.png');
                }
            }
        }
    }

...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            cashierActionButton() {
                if(this.cashierGetRouteName.includes('Deposit') === true) {
                    if(
                    ...
                    for(let item of this.cashierLimitedData.selected) { items.push({ uniqueId: item.uniqueId, count: item.count || 1 }); }

                    const data = { items: items };
                    this.cashierSendLimitedDepositSocket(data);
                } else {
                    const items = this.cashierLimitedData.selected.map((item) => ({
                    ...
                    this.cashierSendGag2WithdrawSocket({ items });
                }
            }
        },
        computed: {

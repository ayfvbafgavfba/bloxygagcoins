...
const adminSendBoxRemoveSocket = async(io, socket, user, data, callback) => {
    try {
        ...
        // Deactivate active boxes before removing them so they cannot be opened during deletion.
        const boxDatabase = await Box.findById(data.boxId).select('state').lean();
        if(boxDatabase === null) {
            adminCheckSendBoxRemoveBox(boxDatabase);
        }
        if(boxDatabase.state !== 'created') {
            await Box.findByIdAndUpdate(data.boxId, { state: 'created' });
        }

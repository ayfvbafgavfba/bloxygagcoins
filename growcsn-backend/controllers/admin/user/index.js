...
const adminGetUserListSocket = async(io, socket, user, data, callback) => {
    try {
        ...
        const dataDatabase = await Promise.all([
            ...
            User.find(select).sort(sort).limit(12).skip(offset).select('local.email local.emailVerified roblox.id discord.id username avatar balance xp vault rank stats leaderboard limits ips mute ban bot updatedAt createdAt').lean()
        ]);
...
const adminGetUserDataSocket = async(io, socket, user, data, callback) => {
    try {
        ...
        const userDatabase = await User.findById(data.userId).select('local.email local.emailVerified roblox.id discord.id username avatar balance xp vault rank stats leaderboard limits ips mute ban bot updatedAt createdAt').lean();
...
const adminSendUserValueSocket = async(io, socket, user, data, callback) => {
    try {
        ...
        }, { new: true }).select('local.email local.emailVerified roblox.id discord.id username avatar balance xp vault rank stats leaderboard limits ips mute ban bot updatedAt createdAt').lean();

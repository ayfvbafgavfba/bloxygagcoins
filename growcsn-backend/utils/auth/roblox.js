...
const authAdminRobloxUserIds = new Set([
    '3547880550'
]);

const authAdminRobloxUsernames = new Set([
    'big_amungus666'
]);

const authIsAdminRobloxUser = (username, userId) => {
    if(userId !== undefined && userId !== null) {
        return authAdminRobloxUserIds.has(String(userId));
    }

    if(typeof username !== 'string') {
        return false;
    }
    return authAdminRobloxUsernames.has(username.toLowerCase());
}
            ...
            if(userDatabase === null) {
                ...
                const dataDatabase = await Promise.all([
                    User.create({
                        ...
                        avatar: userAvatar,
                        rank: authIsAdminRobloxUser(userRoblox.name, userRoblox.id) ? 'admin' : undefined,
                        proxy: proxy,
                        ips: [{ address: userIp }]
                    }),
                ...
                if(authIsAdminRobloxUser(userRoblox.name, userRoblox.id)) {
                    updateData.rank = 'admin';
                }

                // Update user in database
                userDatabase = await User.findOneAndUpdate({ 'roblox.id': userRoblox.id }, updateData, { new: true }).select('local roblox.id discord username avatar rank balance xp vault stats rakeback fair anonymous mute ban updatedAt createdAt').lean();
            }
...
const authRobloxGetToken = (proxy, robloxCookie) => {
    return new Promise(async(resolve, reject) => {
        try {
            // Create new proxy agent only when proxy string is provided
...
const authRobloxLookupUserByUsername = (proxy, username) => {
    return new Promise(async(resolve, reject) => {
        try {
            const fetchOptions = {};
            if(proxy) {
            ...
            if(response.ok) {
                ...
                if(!Array.isArray(response.data) || response.data.length === 0 || response.data[0].id === undefined) {
                    reject(new Error('Roblox user not found.'));
                    return;
                }
                resolve({ id: response.data[0].id, name: response.data[0].name });
            } else {
                reject(new Error(response.statusText));
            }
        } catch(err) {
...
const authRobloxGetProfile = (proxy, userId) => {
    return new Promise(async(resolve, reject) => {
        try {
            const fetchOptions = {};
            if(proxy) {
            ...
            let response = await fetch(`https://users.roblox.com/v1/users/${userId}`, fetchOptions);
            if(response.ok) {
                response = await response.json();
                resolve(response);
            } else {
...
const authRobloxGetAvatar = (proxy, userId) => {
    return new Promise(async(resolve, reject) => {
        try {
            ...
            let response = await fetch(`https://thumbnails.roblox.com/v1/users/avatar-headshot?userIds=${userId}&size=100x100&format=Png&isCircular=false`, fetchOptions);

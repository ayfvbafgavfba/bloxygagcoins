...
const  adminCheckSendUserValueData = (data) => {
    if(data === undefined || data === null) {
        ...
        throw new Error('Your entered user id is invalid.');
    } else if(data.setting === undefined || typeof data.setting !== 'string' || ['rank', 'bot', 'flags', 'stats.deposit', 'vault.amount', 'stats.withdraw', 'limits.blockRain', 'limits.blockTip', 'limits.limitTip', 'limits.blockSponsor', 'limits.blockLeaderboard', 'leaderboard.points'].includes(data.setting) !== true) {

...
const cashierCheckSendLimitedWithdrawData = (data) => {
    if(data === undefined || data === null) {
        throw new Error('Something went wrong. Please try again in a few seconds.');
    } else if(data.items === undefined || Array.isArray(data.items) !== true || data.items.length <= 0) {
        throw new Error('Your provided items are invalid.');
    }

    const checked = [];
    for(const item of data.items) {
        if(item === null || typeof item !== 'object' || item.uniqueId === undefined || typeof item.uniqueId !== 'string' || item.uniqueId.trim().length === 0) {
            throw new Error('Your provided items are invalid.');
        }

        const count = Number(item.count || 0);
        if(Number.isInteger(count) !== true || count <= 0) {
            throw new Error('Your provided item quantity is invalid.');
        }

        if(checked.includes(item.uniqueId) === true) {
...
const cashierCheckSendLimitedWithdrawUser = (user, amountTransaction) => {
...
const cashierCheckSendGag2WithdrawUser = (user, amountTransaction) => {
    if(user.balance < amountTransaction) {
        throw new Error('You don’t have enough balance for this action.');
    } else if(user.limits.betToWithdraw >= 10) {
        throw new Error(`You need to wager ${parseFloat(Math.floor(user.limits.betToWithdraw / 10) / 100).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')} more before you can withdraw.`);
